...
Segment labels MUST support placement along their segment (`start`, `center`, `end`, `auto`) and orientation (`auto`, `upright`, or `along` the segment). `auto` MUST prefer, in order: an upright label above a horizontal run, an upright label below a horizontal run, a label rotated along a vertical run, and an upright label beside a vertical run. Explicit `upright` and `along` values MUST remain hard choices. Rich structured label content MAY be added later, but plain text MUST be supported.

The nearest readable edge of a label SHOULD use the style profile's minimum line-to-label clearance. For an upright multiline label beside a vertical run, text on the right MUST be left-aligned and text on the left MUST be right-aligned; centering those lines is not the default. A label and the run that owns it MUST remain in the same semantic container space. A label pinned to a gap, padding band, or corridor MUST fit inside that region's cross-section and MUST NOT escape across a neighboring container boundary. If it does not fit, layout MUST enlarge the owning whitespace reservation and solve again rather than move the label into another container.
...
- compatible same-direction approaches to one join or bundle front SHOULD place their bends on one common orthogonal coordinate whenever their occupied intervals are disjoint and the shared coordinate is legal in every cited routing region. This is geometric alignment, not positive-length sharing: the approaches retain distinct track identities and a bundle retains its separate terminal lanes;
